...
function Compare() {
  ...
  // Load all available car models from backend
  useEffect(() => {
    fetch(API_URL)
      .then(res => res.json())
  ...
  // Safely retrieve a specific spec value from a car object
  ...
  // Format price to Malaysian Ringgit display
  ...
  return (
    <div className="compare-section">
      ...
      <div className="compare-container">
        ...
        {/* Top row showing selected models and images */}
        <div className="compare-row header-row">

          {/* Left column for VS indicator */}
          <div className="header-col label-col">
            <div className="vs-badge">VS</div>
          </div>

          {/* First model selection column */}
            ...
            >
              ...
              {cars.map(c => (
                <option
                  key={c.id}
                  value={c.id}
                  disabled={c.id === parseInt(selectedId2)}
                >
                  {c.modelName}
                </option>
              ))}
            ...
            {car1 ? (
              <>
                <div className="image-container">
                  <img
                    src={car1.sideImageUrl}
                    alt={car1.modelName}
                    className="car-image fade-in"
                  />
                </div>
              ...
              <div className="image-container" style={{ opacity: 0.3, color: '#555' }}>
                NO MODEL SELECTED
              </div>
            )}
          </div>

          {/* Second model selection column */}
          <div className="header-col">
            ...
            >
              ...
              {cars.map(c => (
                <option
                  key={c.id}
                  value={c.id}
                  disabled={c.id === parseInt(selectedId1)}
                >
                  {c.modelName}
                </option>
              ))}
        ...
        {/* Comparison data shown only when both models are selected */}
        {car1 && car2 && (
          <>
